...
task :geraIndices => :environment do
  ...
  bairros = ["ASA SUL", "ASA NORTE", "LAGO SUL", "LAGO NORTE", "SUDOESTE"]
...
=begin  
  for numqt in (1..5)
    ...
    ###################
    #Indices de apts por bairro
    ###################
    bairros.each do |item_bairro|
      resultado = connection.execute("select avg(VALOR), avg(VALM2) from imovels where numqt=#{numqt} and bairro= '#{item_bairro}' ")  
      val_med = resultado.to_s.to_i
      if (val_med != 1)
        val_med /=1000
        res_m2 =  resultado.to_s.slice(resultado.to_s.index("M2")+3..resultado.to_s.index("VALOR"))
        val_med_m2 = res_m2.slice(0..res_m2.index(".")-1)     
        puts " VALOR MEDIO DE #{numqt}qt DO #{item_bairro} = #{val_med} e VALOR MEDIO M2 = #{val_med_m2}"
        Util.escreveIndice(item_bairro, "", numqt, val_med, val_med_m2)
      end
    end
  end  
end
